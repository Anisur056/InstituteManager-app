...
class TblFingerDevice extends Controller
{
    ...
    {
        $zk = new ZktecoLib('DEVICE_IP', 'DEVICE_PORT');

        if ($zk->connect()) {
            $zk->disableDevice();

            User::chunk(100, function ($users) use ($zk) {
                foreach ($users as $user) {
                    $zk->setUser(
                        (int) $user->id,          // Unique ID for the device (max 65535)
                        (string) $user->employee_id, // Employee ID for the device
                        (string) $user->name,
                        '',                      // Password (optional)
                        ZktecoLib::LEVEL_USER,   // User level
                        (string) $user->card_number // Card number (optional)
                    );
                }
            });
            ...
            $zk->enableDevice();
            $zk->disconnect();
            return "Users successfully synchronized to ZKTeco device.";
        } else {
            return "Failed to connect to ZKTeco device.";
        }
    }
