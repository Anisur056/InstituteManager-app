        ...
        <div class="card-body">
            <form action="{{ route('shifts.update',$data->id) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input  value="{{ $data->shift_name_en }}" 
                            type="text" 
                            class="form-control @error('shift_name_en') is-invalid @enderror"   
                            name="shift_name_en">
                    <span class="text-danger"> @error('shift_name_en') {{$message}} @enderror </span>
                </div>
                <div class="mb-3">
                    <label class="form-label">শিফটের নাম</label>
                    <input  value="{{ $data->shift_name_bn }}" 
                            type="text" 
                            class="form-control @error('shift_name_bn') is-invalid @enderror" 
                            name="shift_name_bn">
                    <span class="text-danger"> @error('shift_name_bn') {{$message}} @enderror </span>
                </div>
                <div class="mb-3">
